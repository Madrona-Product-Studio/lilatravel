  ...
  return (
    <div style={{ position: "relative", zIndex: open ? 95 : "auto", background: C.stone, borderBottom: `1px solid ${C.stone}` }}>
      ...
      <button onClick={() => setOpen(!open)} style={{ width: "100%", border: "none", cursor: "pointer", background: "transparent", padding: isMobile ? "14px 20px" : "14px 52px", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(0,0,0,0.045)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
        ...
        <span style={{ fontFamily: "'Quicksand', sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "#5c6358", flexShrink: 0 }}>{"Olympic Right Now"}</span>
        {!isMobile && teasers.length > 0 && (<span style={{ fontFamily: "'Quicksand', sans-serif", fontSize: 11, fontWeight: 600, color: "#6b6359", letterSpacing: "0.04em" }}>— {teasers.map((t, i) => (
              <span key={i}>
                {i > 0 && <span style={{ margin: "0 10px", opacity: 0.55, fontWeight: 300 }}>|</span>}
                {t}
              </span>
            ))}</span>)}
        {isMobile && weather && (<span style={{ fontFamily: "'Quicksand', sans-serif", fontSize: 11, fontWeight: 600, color: "#6b6359", letterSpacing: "0.04em" }}>· {weather.temp}° · {moon?.name}</span>)}
