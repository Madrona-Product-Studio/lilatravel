...
export default async function handler(req, res) {
  ...
  const subject = isSave
    ? `Your Lila Trips itinerary — ${itineraryTitle || 'pick up where you left off'}`
  ...
  const html = isSave ? `
    <div style="font-family: 'Helvetica Neue', sans-serif; max-width: 520px; margin: 0 auto; padding: 40px 24px; color: #1E2825;">
      ...
      <h1 style="font-size: 24px; font-weight: 600; margin-bottom: 12px; line-height: 1.3;">${itineraryTitle || 'Your itinerary is waiting'}</h1>
      ...
      <a href="${itineraryUrl}" style="display: inline-block; background: #1E2825; color: #FAF8F4; font-size: 12px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; padding: 14px 28px; text-decoration: none; border-radius: 2px;">
        Open my itinerary →
      </a>
      <p style="font-size: 12px; color: #7A857E; margin-top: 40px; line-height: 1.6;">
        Or copy this link: <a href="${itineraryUrl}" style="color: #4A8C85;">${itineraryUrl}</a>
      </p>
    </div>
  ` : `
    <div style="font-family: 'Helvetica Neue', sans-serif; max-width: 520px; margin: 0 auto; padding: 40px 24px; color: #1E2825;">
      ...
      <p style="font-size: 15px; color: #4A5650; line-height: 1.75; margin-bottom: 32px;">
        ${itineraryTitle ? `"${itineraryTitle}" —` : ''} take a look and see what's been put together.
      </p>
      <a href="${itineraryUrl}" style="display: inline-block; background: #1E2825; color: #FAF8F4; font-size: 12px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; padding: 14px 28px; text-decoration: none; border-radius: 2px;">
        View itinerary →
      </a>
