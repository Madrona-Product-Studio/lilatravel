...
- [x] **Card Deck system** (Apr 2026) — 30-card practice deck replaces the AI-generated mindfulness pick. Five-principle taxonomy (Presence / Oneness / Flow / Compassion / Reverence). Implemented per `claude-code-brief-card-deck.md`:
  ...
  - [x] Five tradition cards (Apr 2026) — new TraditionScreen type inserted after orientation, before chapters. Each card: sage-green gradient background, symbol, origin, name (Quicksand bold), essence (Cormorant italic), history, three key concepts (label + desc), "In the wild" section. SectionLabel pattern with flanking hairlines. Total screens: 42.
