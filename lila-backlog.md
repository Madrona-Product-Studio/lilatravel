...
- [x] **Card Deck system** (Apr 2026) — 30-card practice deck replaces the AI-generated mindfulness pick. Five-principle taxonomy (Presence / Oneness / Flow / Compassion / Reverence). Implemented per `claude-code-brief-card-deck.md`:
  ...
  - [x] Directional card navigation (Apr 2026) — forward: current card deals off left (dealOff 0.42s), next card revealed stationary underneath. Back: previous card stacks on from left (stackOn 0.44s), base unchanged until landed. Two-layer rendering (base + anim layer).
